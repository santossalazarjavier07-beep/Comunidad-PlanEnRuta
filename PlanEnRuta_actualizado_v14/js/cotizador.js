...
function actualizarTotalCotizador() {
    ...
    /* "No incluye": en los paquetes completos (tour + hospedaje) no se muestra el hospedaje */
    const noIncluye = paqSel ? data.noIncluye.filter(i => !/hospedaje/i.test(i)) : data.noIncluye;
    modal.querySelector("#cotNoIncluye").innerHTML = noIncluye.map(i => `<li>${i}</li>`).join("");
